...
TEST_F(StateTest, AfterWordTest) {
  ...
  EXPECT_EQ("[fork1_1,thinking2_1,tmp1_2_1]", state->After(glf1_grf1_plf1)->hash());
...
TEST_F(StateTest, ExpandTest) {
  ...
  EXPECT_EQ("[fork1_1,thinking1_1,tmp2_1_1]", succ1.hash());

  EntitySet new_entities2;
  state->Expand(get_left_fork1_, &new_entities2);
  State succ2(new_entities2);
  EXPECT_EQ("[fork2_1,thinking2_1,tmp1_1_1]", succ2.hash());
}
